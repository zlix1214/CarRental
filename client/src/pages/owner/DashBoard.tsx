...
import type { Booking, OwnerDashboardData } from "../../types/domain";
...
const emptyDashboardData: OwnerDashboardData = {
  totalCars: 0,
  totalBookings: 0,
  pendingBookings: 0,
  completedBookings: 0,
...
const Dashboard = () => {
  ...
  const data = dashboardQuery.data?.success
  ...
  useEffect(() => {
    if (token && dashboardQuery.data && !dashboardQuery.data.success) {
      setShowModal(true);
    }
  }, [dashboardQuery.data, token]);

  useEffect(() => {
    if (dashboardQuery.error) {
      toast.error(getErrorMessage(dashboardQuery.error));
    }
